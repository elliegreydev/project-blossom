...
export default function CareOverviewPage() {
  ...
  return <SensitiveModuleGate><div className={styles.screen}>
    <ScreenHeader title="Care overview" backHref="/care" />
    ...
    <section className={styles.section} style={{ borderTop: "none", paddingTop: 0 }}><div className={styles.sectionTitle}>Next up</div>
      {nextDose ? <Link href="/care/medication" className={styles.item}><span className={styles.itemTitle}>{nextDose.medication.name}</span><span className={styles.itemMeta}>Scheduled for {when(nextDose.scheduledTime)}</span></Link> : <div className={styles.empty}><div className={styles.emptyTitle}>No medication due next</div><div className={styles.emptySubtitle}>Medication only appears here when you have a schedule set.</div></div>}
      {nextAppointment ? <Link href="/plan" className={styles.item}><span className={styles.itemTitle}>{nextAppointment.title}</span><span className={styles.itemMeta}>{when(nextAppointment.appointmentAt)}</span></Link> : <div className={styles.empty}><div className={styles.emptyTitle}>No appointment coming up</div><div className={styles.emptySubtitle}>Future appointments will appear here when you add them.</div></div>}
    </section>

    <section className={styles.section}><div className={styles.itemRow}><div className={styles.sectionTitle}>Supplies</div><Link href="/care/medication" className={styles.linkButton}>Review</Link></div>
      {supplyHeadsUps.length === 0 ? <div className={styles.empty}><div className={styles.emptyTitle}>Nothing needs checking</div><div className={styles.emptySubtitle}>Supply heads-ups appear only when they may help.</div></div> : <div className={styles.list}>{supplyHeadsUps.map((supply) => <Link href="/care/medication" key={supply.id} className={styles.item}><span className={styles.itemTitle}>{supply.title}</span><span className={styles.itemMeta}>{supply.meta}</span></Link>)}</div>}
    </section>

    <section className={styles.section}><div className={styles.itemRow}><div className={styles.sectionTitle}>Recent blood tests</div><Link href="/care/blood-tests" className={styles.linkButton}>View all</Link></div>
      {!profile.enabledModules.includes("bloodTests") ? <p className={styles.sectionNote}>Blood tests are currently turned off in your modules.</p> : bloodTests.length === 0 ? <div className={styles.empty}><div className={styles.emptyTitle}>Nothing recorded yet</div><div className={styles.emptySubtitle}>This is only a private record of results you choose to add.</div></div> : <div className={styles.list}>{bloodTests.slice(0, 3).map((test) => <Link href="/care/blood-tests" key={test.id} className={styles.item}><span className={styles.itemTitle}>{test.testName}</span><span className={styles.itemMeta}>{test.date}{test.value ? ` · ${test.value}${test.unit ? ` ${test.unit}` : ""}` : ""}</span></Link>)}</div>}
    </section>
