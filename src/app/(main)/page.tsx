...
const SHORTCUTS: Record<HomeShortcutKey, { label: string; href: string }> = {
  medication: { label: "Medication", href: "/care/medication" },
  calendar: { label: "Plan", href: "/plan" },
  journal: { label: "Journal & check-ins", href: "/care/journal" },
  goals: { label: "Goals", href: "/care/goals" },
...
export default function HomePage() {
  ...
  const dueDoses = meds.filter((med) => med.active).flatMap((med) => dueDosesToday(med, now)
    ...
    .map((slot) => ({ id: med.id + slot, label: med.name, meta: timeLabel(slot), href: "/care/medication", overdue: now.getTime() - new Date(slot).getTime() > OVERDUE_THRESHOLD_MS })));
  const todayAppts = appts.filter((appointment) => {
    const time = new Date(appointment.appointmentAt);
    return time >= now && time <= todayEnd;
  }).map((appointment) => ({ id: appointment.id, label: appointment.title, meta: timeLabel(appointment.appointmentAt), href: "/plan", overdue: false }));
