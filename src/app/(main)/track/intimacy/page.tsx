    ...
    <SensitiveModuleGate>
      <div className={feature.screen}>
        ...
        <div className={feature.section}>
          {entries.length === 0 ? <div className={feature.empty}><div className={feature.emptyTitle}>Nothing needs recording to be real</div><div className={feature.emptySubtitle}>If you ever want a private note for yourself, this space is here.</div></div> : <div className={feature.list}>{entries.map((entry) => <article key={entry.id} className={feature.item}><button type="button" className={styles.entryButton} onClick={() => setEditing(entry)}><div className={feature.itemRow}><span className={feature.itemTitle}>{entry.label ?? "Private entry"}</span><span className={feature.itemMeta}>{dateLabel(entry)}</span></div>{entry.feeling && <span className={styles.feeling}>{FEELING_LABELS[entry.feeling]}</span>}{entry.tags.length > 0 && <span className={feature.itemMeta}>{entry.tags.join(" · ")}</span>}{entry.privateNote && <span className={styles.preview}>{entry.privateNote}</span>}</button>{(entry.feeling === "unsure" || entry.feeling === "not-good") && <Link href="/crisis-support" className={styles.supportLink}>Need support right now?</Link>}<div className={styles.actions}><button type="button" className={feature.linkButton} onClick={() => setEditing(entry)}>Edit</button><button type="button" className={feature.linkButton} onClick={() => void deleteIntimacyEntry(entry.id)}>Remove</button></div></article>)}</div>}
