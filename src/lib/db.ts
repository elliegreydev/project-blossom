...
function createDb(): BlossomDb {
  ...
  // entries on next load rather than leaving them sitting unreachable now
  // that the tab is gone. Local-only data (never synced), so this only
  // needs to happen on-device.
  instance.version(19).stores({
    ...
    await tx.table("euphoriaEntries").clear();
  });
  // Social Transition Planner is being pulled back for rework - clears any
  // people/plans/tasks on next load rather than leaving them sitting
  // unreachable now that the route is gone. Local-only data (never synced),
  // so this only needs to happen on-device.
  instance.version(20).stores({
    ...
    await tx.table("socialTransitionPeople").clear();
    await tx.table("socialTransitionPlans").clear();
    await tx.table("socialTransitionTasks").clear();
  });
  // Gentle Mode is being pulled back for rework - resets the preference on
  // next load rather than leaving it silently on for a feature that's no
  // longer reachable in Settings. gentle_mode is synced, so the server-side
  // column was already reset directly (see reset_gentle_mode.sql); this
  // just keeps a device that hasn't synced yet consistent too.
  instance.version(21).stores({
    ...
    await tx.table("profiles").toCollection().modify((profile: Profile) => {
      profile.gentleMode = false;
    });
  });
  // Accessibility Profiles is being pulled back for rework - resets the
  // preset choice and the four toggles it introduced (reduceMotion and
  // textSize predate it and are untouched). Not synced, so this only needs
  // to happen on-device.
  instance.version(22).stores({
    ...
    await tx.table("profiles").toCollection().modify((profile: Profile) => {
      profile.accessibilityProfile = "custom";
      profile.highContrast = false;
      profile.largeTouchTargets = false;
      profile.readingComfort = false;
